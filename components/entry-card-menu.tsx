...
/**
 * Files a title into one of the viewer's collections.
 *
...
export function EntryCardMenu({
  ...
  return (
    <ContextMenuContent className="w-56">
      ...
      {/* Filing from the shelf, alongside the picker on the collection page.
          The picker is the surface for building a list deliberately; this is
          for the title already under the pointer.
          ...
          collection ▸" would be unreachable in tests and awkward under a
          long-press. A collection already holding the title stays listed,
          ticked and disabled — dropping it would make "already in it" and "no
          such collection" look the same. */}
      {collectionOptions.length > 0 ? <ContextMenuSeparator /> : null}
      {collectionOptions.map((collection) => (
        <ContextMenuItem
          key={collection.id}
          disabled={collection.has || isPending}
          onSelect={() => run(() => submitAddToCollection(entry, collection.id))}
        >
